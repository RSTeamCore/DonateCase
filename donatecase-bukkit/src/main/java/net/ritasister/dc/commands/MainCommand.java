package net.ritasister.dc.commands;

import net.ritasister.dc.DonateCase;
import net.ritasister.dc.commands.executor.DCCommand;
        ...
        if (sender.hasPermission("DonatCase.Admin")) {
            if (args.length >= 1) {
                if (!args[0].equalsIgnoreCase("create") && (!args[0].equalsIgnoreCase("delete") || !(sender instanceof Player))) {
                    ...
                    final boolean cr = args[0].charAt(0) == 'c';
                    final Player p = (Player)sender;
                    final Location l = p.getTargetBlock(null, 5).getLocation();
                    if (cr) {
                        if (args.length >= 2) {
