...
public class StartAnimation {
    ...
    public StartAnimation(final Player player, final @NotNull Location location, final Case c) {
                    ...
                    if (this.i == 16) {
                        ...
                        if (DonateCase.t.hasLevelGroup(player, winGroup.getGroup())) {
                            for (final String cmd : c.getCmds()) {
                                Bukkit.dispatchCommand(Bukkit.getConsoleSender(), DonateCase.t.rt(cmd, "%player:" + player.getName(), "%group:" + winGroup.getGroup()));
                            }
